...
//        })
    ...
    func deleteAll() {
        let d = news_title.delete()
        try! sqlManager.database.run(d)
    }
    
    func delete(_ newsTitle: HomeNewsTitle) {
        if exist(newsTitle) {
            let title = news_title.filter(category == newsTitle.category.rawValue)
            // 更新数据
            //            try! sqlManager.database.run(title.delete())
            do {
                if try sqlManager.database.run(title.delete()) > 0 {
                    //                    print("deleted")
                } else {
                    print("row not found")
                }
